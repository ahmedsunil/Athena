...
> Depends on: `00-shared-models.md` (Staff for teacher incharges, house masters, scout leaders)
...
| assistant_leader_name | string | yes | student or staff assistant leader name |
| is_active | boolean | no | default true |
| sort_order | unsignedInteger | no | default 0 |
| created_at | timestamp | yes | |
| updated_at | timestamp | yes | |
...
{
    return $this->belongsTo(Staff::class, 'teacher_incharge_id');
}
```
...
- **Teacher incharges and house masters** are FK to `Staff` — these are teachers who hold the role semi-permanently.
...
- Query pattern for student life page:
  ```php
  Club::with('teacherIncharge')->where('is_active', true)->orderBy('sort_order')->get();
  House::with('houseMaster')->where('is_active', true)->orderBy('sort_order')->get();
  Prefect::where('is_active', true)->orderBy('sort_order')->get();
  ScoutGroup::where('is_active', true)->orderBy('sort_order')->get();
  ```
