...
<div class="mx-auto space-y-4"
     x-data="{
        editing: $wire.entangle('isEditing'),
        tab: new URLSearchParams(window.location.search).get('tab') || 'slides'
     }"
                    ...
                    <div class="grid gap-4 sm:grid-cols-2">
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <textarea wire:model="slides.{{ $index }}.subtitle" rows="3"
                                      class="{{ $textareaClass }}" :disabled="!editing"></textarea>
        ...
        <section x-show="tab === 'principal'" x-cloak class="rounded-xl border border-zinc-200 bg-white p-4 shadow-sm">
            <div class="grid gap-4 sm:grid-cols-2">
                <div>
                    ...
                    <input wire:model="principal.name" class="{{ $inputClass }}" :disabled="!editing">
                    @error('principal.name') <p class="{{ $errorClass }}">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label class="{{ $labelClass }}">Title</label>
                    <input wire:model="principal.title" class="{{ $inputClass }}" :disabled="!editing">
                    @error('principal.title') <p class="{{ $errorClass }}">{{ $message }}</p> @enderror
                </div>
                <div class="sm:col-span-2">
                    <label class="{{ $labelClass }}">Photo URL</label>
                    <input wire:model="principal.photoUrl" class="{{ $inputClass }}" :disabled="!editing">
            ...
            <div class="rounded-xl border border-zinc-200 bg-white shadow-sm">
                @forelse($events as $event)
                    <label wire:key="event-option-{{ $event->public_id }}"
                           :class="editing ? 'cursor-pointer hover:bg-zinc-50' : ''"
                           class="flex items-start gap-3 border-b border-zinc-100 p-4 last:border-0">
                        <input type="checkbox"
        ...
        <section x-show="tab === 'contact'" x-cloak class="rounded-xl border border-zinc-200 bg-white p-4 shadow-sm">
            <div class="grid gap-4 sm:grid-cols-2">
                <div class="sm:col-span-2">
                    ...
                    <input wire:model="contact.address" class="{{ $inputClass }}" :disabled="!editing">
                    @error('contact.address') <p class="{{ $errorClass }}">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label class="{{ $labelClass }}">Phone</label>
                    <input wire:model="contact.phone" class="{{ $inputClass }}" :disabled="!editing">
                    @error('contact.phone') <p class="{{ $errorClass }}">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label class="{{ $labelClass }}">Email</label>
                    <input type="email" wire:model="contact.email" class="{{ $inputClass }}" :disabled="!editing">
