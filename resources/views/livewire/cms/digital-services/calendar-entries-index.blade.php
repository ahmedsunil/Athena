<div class="space-y-4">
    ...
    <div class="flex items-center gap-3 flex-wrap">
        <select wire:model.live="calendarId"
                class="h-9 rounded-md border border-zinc-200 bg-white px-3 text-sm text-zinc-950 shadow-sm focus:border-zinc-950 focus:outline-none focus:ring-1 focus:ring-zinc-950">
            <option value="">— Select calendar —</option>
            @foreach($calendars as $cal)
                <option value="{{ $cal->id }}">{{ $cal->title }}{{ $cal->is_active ? ' (active)' : '' }}</option>
            @endforeach
        </select>
        <a href="{{ route('cms.digital-services.calendars') }}"
           class="text-xs font-medium text-zinc-500 hover:text-zinc-950">Manage calendars →</a>
    </div>
    ...
    @if($currentCalendar)
        ...
        <x-admin.tables.data-table>
            ...
            <x-slot name="body">
                ...
                @empty
                    <tr>
                        <td colspan="6" class="admin-table-cell text-center text-zinc-400">No entries yet. Add one above.</td>
                    </tr>
                @endforelse
            </x-slot>
            <x-slot name="mobile">
    ...
    @else
        <div class="rounded-xl border border-zinc-200 bg-white p-8 text-center text-sm text-zinc-400">
            No calendar selected. Choose one above or
            <a href="{{ route('cms.digital-services.calendars') }}" class="text-zinc-700 underline">create one</a>.
        </div>
    @endif
