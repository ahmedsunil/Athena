<div>
            ...
            <div class="flex items-start gap-4">
                ...
                <div class="min-w-0">
                    <div class="mb-2 flex flex-wrap items-center gap-2">
                        ...
                        <span class="text-xs font-medium text-slate-400">Created {{ $announcement->created_at->format('j M Y') }}</span>
                        @if($announcement->deadline)
                            <span class="rounded-full bg-amber-50 px-2.5 py-1 text-xs font-bold text-amber-700">Deadline {{ $announcement->formatted_deadline }}</span>
                        @endif
                    </div>
    ...
    <main class="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8">
        <div class="rounded-2xl border border-slate-200 bg-white p-6">
            ...
            @else
                <p class="text-slate-400">No additional details provided.</p>
            @endif

            @if($announcement->attachment_links)
                <div class="mt-8 border-t border-slate-100 pt-6">
                    <h2 class="mb-3 text-sm font-bold uppercase tracking-widest text-slate-400">Attached Files</h2>
