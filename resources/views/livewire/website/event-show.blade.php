<div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <h1 class="mb-6 text-2xl font-black text-slate-900 sm:text-3xl">{{ $event->title }}</h1>
        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-7 sm:p-9">
            ...
            <div class="mt-8 border-t border-slate-100 pt-6">
                <p class="mb-4 text-xs font-bold uppercase tracking-widest text-slate-500">{{ __('event_show_details_heading') }}</p>
                <dl class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    <div class="flex items-start gap-2.5">
                        ...
                        <div>
                            <dt class="text-xs text-slate-400">{{ __('event_show_label_location') }}</dt>
                    ...
                    <div class="flex items-start gap-2.5">
                        ...
                        <div>
                            <dt class="text-xs text-slate-400">{{ __('event_show_label_date') }}</dt>
                            <dd class="text-sm font-semibold text-slate-900">{{ $event->formatted_date_range }}</dd>
                        </div>
                    </div>
                    @if($event->contact)
                        <div class="flex items-start gap-2.5">
                            <svg class="w-4 h-4 text-slate-400 mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
                            <div>
                                <dt class="text-xs text-slate-400">{{ __('event_show_label_contact') }}</dt>
                                <dd class="text-sm font-semibold text-slate-900">{{ $event->contact }}</dd>
                            </div>
                        </div>
                    @endif
                </dl>
        ...
        @if($event->attachments && count($event->attachments) > 0)
            <div class="mt-8">
                <p class="text-xs font-bold uppercase tracking-widest text-slate-500 mb-3">{{ __('event_show_downloads') }}</p>
                <div class="space-y-2">
