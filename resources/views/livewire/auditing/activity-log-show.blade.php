<div>
    ...
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div class="flex min-w-0 items-start gap-3">
            ...
            <div class="min-w-0">
                <div class="flex flex-wrap items-center gap-2">
                    <h1 class="admin-page-title">Activity #{{ $activity->id }}</h1>
        ...
        <a href="{{ route('app.activity') }}"
           class="inline-flex items-center justify-center rounded-lg border border-zinc-200 px-3 py-2 admin-label transition hover:bg-zinc-50 hover:text-zinc-950">
            Back to Log
        </a>
    </div>
    ...
    <div class="grid gap-6 lg:grid-cols-3">
        <div class="space-y-6 lg:col-span-2">
            <section class="overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-sm">
                <div class="border-b border-zinc-100 px-5 py-4">
                    <h2 class="admin-section-title">Change Summary</h2>
                    <p class="mt-1 admin-caption">Field-level before and after values captured for this activity.</p>
                </div>
                ...
                @else
                    <div class="px-5 py-12 text-center">
                        ...
                        <p class="text-sm font-medium leading-5 text-zinc-600">No field-level changes were stored.</p>
                        <p class="mt-1 admin-muted">The activity still has event, actor, subject, and metadata details below.</p>
                    </div>
                @endif
            </section>
            ...
            <section class="overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-sm">
                <div class="border-b border-zinc-100 px-5 py-4">
                    <h2 class="admin-section-title">Additional Properties</h2>
                    <p class="mt-1 admin-caption">Extra metadata stored with the audit entry outside of old and new attributes.</p>
                </div>
                    ...
                    <div class="divide-y divide-zinc-100">
                        @foreach($extraProperties as $key => $value)
                            <div class="grid gap-3 px-5 py-4 sm:grid-cols-4">
                                <div>
                                    <p class="admin-link-label text-zinc-950">{{ $key }}</p>
                ...
                @else
                    <div class="px-5 py-8 admin-body-muted">No additional properties were stored for this activity.</div>
                @endif
            </section>
            ...
            @if(count($rawAttributeChanges))
                <section class="overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-sm">
                    <div class="border-b border-zinc-100 px-5 py-4">
                        <h2 class="admin-section-title">Raw Attribute Changes</h2>
                        <p class="mt-1 admin-caption">Database-level attribute_changes payload for troubleshooting.</p>
                    </div>
        ...
        <aside class="space-y-4">
            ...
            <section class="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm">
                <h2 class="admin-section-title">Actor</h2>
                <div class="mt-4 flex items-start gap-3">
                    ...
                    <div class="min-w-0">
                        ...
                        @if($causerEmail)
                            <p class="truncate admin-caption">{{ $causerEmail }}</p>
                        @endif
            ...
            <section class="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm">
                <h2 class="admin-section-title">Subject</h2>
                <dl class="mt-4 space-y-3 text-xs">
                    <div class="flex items-start justify-between gap-3">
            ...
            <section class="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm">
                <h2 class="admin-section-title">Technical Details</h2>
                <dl class="mt-4 space-y-3 text-xs">
                    <div class="flex items-center justify-between gap-3">
