<div class="rounded-xl border border-zinc-200 bg-white p-6 shadow-sm">
    <div class="mb-6 flex items-start justify-between gap-4">
        <div>
            <h3 class="text-sm font-semibold text-zinc-950">Two-Factor Authentication</h3>
            <p class="mt-1 text-xs text-zinc-500">Add extra security with an authenticator app.</p>
        </div>
        @if($enabled)
    ...
    @if($enabled)
        ...
        @if($showingRecoveryCodes && count($recoveryCodes))
            <div class="mb-4 rounded-xl border border-zinc-200 bg-zinc-50 p-4">
                <p class="mb-2 text-xs font-medium text-zinc-700">Store these recovery codes in a safe place.</p>
    ...
    @elseif($showingConfirmation)
        ...
        <div class="space-y-3">
            <div>
                <label class="mb-1.5 block text-xs font-medium text-zinc-700">Confirmation code</label>
            ...
            <div class="flex gap-2">
                <button wire:click="confirmTwoFactor"
                        class="rounded-lg bg-zinc-950 px-4 py-2 text-xs font-semibold text-white shadow-sm hover:bg-zinc-800">
                    Confirm & enable
                </button>
                <button wire:click="disableTwoFactor"
