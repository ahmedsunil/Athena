...
class Dashboard extends Component
{
    ...
    {
        [$start, $end] = $this->dateBounds($from, $to);
        $roles = User::whereBetween('created_at', [$start, $end])
            ->selectRaw('role, count(*) as total')
            ->groupBy('role')
            ->orderByDesc('total')
            ->get()
            ->map(fn ($role) => [
                'label' => ucfirst($role->role ?: 'user'),
    ...
    private function componentUsage(string $from, string $to): array
    {
        ...
        $admins = User::whereBetween('created_at', [$start, $end])->where('role', 'admin')->count();
